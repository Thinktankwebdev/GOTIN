...
setProgressBar = function ($el, score) {
	...
	if (options.showVerdicts) {
		if (options.viewports.verdict) {
			...
			if ($verdict.length === 0) {
			    $verdict = $('<span class="password-verdict" style="display: inline-block;"> </span>');
	...
	if (score < options.scores[0]) {
		...
		progressbar.find(".bar").attr("score", "5");
		if (options.showVerdicts) {
			$verdict.html('<span style="color: #aaa;"> Password Strength: </span> ' + options.verdicts[0]);
		}
	} else if (score >= options.scores[0] && score < options.scores[1]) {
		progressbar.addClass("progress-danger").removeClass("progress-warning").removeClass("progress-success");
		progressbar.find(".bar").css("width", "25%");
		progressbar.find(".bar").attr("score", "25");
		if (options.showVerdicts) {
			$verdict.html('<span style="color: #aaa;"> Password Strength: </span> ' + options.verdicts[1]);
		}
		...
		progressbar.find(".bar").attr("score", "50");
		if (options.showVerdicts) {
			$verdict.html('<span style="color: #aaa;"> Password Strength: </span> ' + options.verdicts[2]);
		}
	} else if (score >= options.scores[2] && score < options.scores[3]) {
		progressbar.addClass("progress-warning").removeClass("progress-danger").removeClass("progress-success");
		progressbar.find(".bar").css("width", "75%");
		progressbar.find(".bar").attr("score", "75");
		if (options.showVerdicts) {
			$verdict.html('<span style="color: #aaa;"> Password Strength: </span> ' + options.verdicts[3]);
		}
		...
		progressbar.find(".bar").attr("score", "100");
		if (options.showVerdicts) {
			$verdict.html('<span style="color: #aaa;"> Password Strength: </span> ' + options.verdicts[4]);
		}
	}
},
...
progressWidget = function () {
	return '<div class="progress strength_"><div class="bar"></div></div>';
},
	...
	init: function (settings) {
		...
		return this.each(function (idx, el) {
			...
			if (allOptions.showVerdicts) {
				verdict = $('<span class="password-verdict"> <span style="color: #aaa;"> Password Strength: </span> ' + allOptions.verdicts[0] + '</span>');
